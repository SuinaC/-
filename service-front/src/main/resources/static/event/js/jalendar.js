...
$(function () {    
    (function ($) {
        $.fn.jalendar = function (options) {
            ...
            var lTotalEvents = {};
            var lEvent = {};
            dayNames['CN'] = new Array('星期一', '星期二', '星期三', '星期四', '星期五', '星期六', '星期日');
            ...
            $this.on('click', '.this-month', function () {
                ...
                $this.children('.jalendar-wood, .wood-bottom').animate({width : '+=332px' }, 200, function() {
            ...
            $this.find('.submit').on('click', function(){
                ...
                calcTotalDayAgain();
                calcScroll();
                var thisd = thisDay.split("/");
                var times = thisd[2] + "/" + thisd[1] + "/" + thisd[0] + "/" + time;
                var dateId = thisd[2] + "/" + thisd[1] + "/" + thisd[0] + "/" + dataId;
                $.post(
                    "/consumerInformation/insertCalendar",
                    "time="+times+"&title="+title+"&dateId="+dateId,
                    function (data){
